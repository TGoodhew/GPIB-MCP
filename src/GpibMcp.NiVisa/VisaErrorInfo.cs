...
namespace GpibMcp.Instruments
{
    ...
    {
        private static readonly GpibStatus None = GpibStatus.Empty;

        /// <summary>Decodes <paramref name="ex"/>; returns <see cref="GpibStatus.Empty"/> if it is not a VISA error.</summary>
        public static GpibStatus Describe(Exception ex)
        {
        ...
        public static GpibStatus DescribeCode(int code)
        {
            GpibStatus info;
            if (Codes.TryGetValue(code, out info)) return new GpibStatus(info.Name, info.Meaning, code);
            return new GpibStatus("VISA 0x" + code.ToString("X8"),
        ...
        private static readonly Dictionary<int, GpibStatus> Codes = new Dictionary<int, GpibStatus>
        {
            [Tmo] = new GpibStatus("VI_ERROR_TMO",
                "Timeout - the instrument did not respond in time. Check it is powered on, set to this address, and not busy."),
            [unchecked((int)0xBFFF005F)] = new GpibStatus("VI_ERROR_NLISTENERS",
                "No listener on the bus - nothing acknowledged at this address. Check the GPIB address, power, and cabling."),
            [unchecked((int)0xBFFF0011)] = new GpibStatus("VI_ERROR_RSRC_NFOUND",
                "Resource not found - no instrument or board matches this resource string."),
            [unchecked((int)0xBFFF000E)] = new GpibStatus("VI_ERROR_INV_RSRC_NAME",
                "Invalid resource name - the VISA resource string is malformed."),
            [unchecked((int)0xBFFF0072)] = new GpibStatus("VI_ERROR_RSRC_BUSY",
                "Resource busy - the instrument or session is currently in use."),
            [unchecked((int)0xBFFF0023)] = new GpibStatus("VI_ERROR_ABORT",
                "The operation was aborted."),
            [unchecked((int)0xBFFF003E)] = new GpibStatus("VI_ERROR_IO",
                "General I/O error on the bus."),
            [unchecked((int)0xBFFF00A6)] = new GpibStatus("VI_ERROR_CONN_LOST",
                "Connection to the instrument was lost."),
            [unchecked((int)0xBFFF0036)] = new GpibStatus("VI_ERROR_NCIC",
                "The controller is not Controller-In-Charge of the GPIB bus."),
            [unchecked((int)0xBFFF000A)] = new GpibStatus("VI_ERROR_INV_OBJECT",
                "Invalid or already-closed session/object."),
            [unchecked((int)0xBFFF0067)] = new GpibStatus("VI_ERROR_NSUP_OPER",
                "Operation not supported by this resource."),
            [unchecked((int)0xBFFF001D)] = new GpibStatus("VI_ERROR_INV_SETUP",
