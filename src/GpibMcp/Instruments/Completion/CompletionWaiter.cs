...
namespace GpibMcp.Instruments.Completion
{
    ...
    /// <summary>
    /// The data-driven, hardware-agnostic completion state machine (issue #17). Decoupled from VISA
    /// and the MCP layer via <see cref="IStatusChannel"/> and injected clock/sleep so it can be unit-
    /// and harness-tested headlessly. 3-state dispatch (refuse / needs-definition / run); the run flow
    /// pre-clears stale status, arms the SRQ mask, starts the operation, and confirms completion by
    /// polling the LATCHED status byte (reliable - bits stay set until read), then clears/restores.
    /// </summary>
    public static class CompletionWaiter
    {
            ...
            int pollIntervalMs = DefaultPollIntervalMs)
        {
            ...
            channel.Send(model.EnableMask.SetCommand.Replace("{mask}", mask.ToString(CultureInfo.InvariantCulture)));
            if (!string.IsNullOrEmpty(op.Arm)) channel.Send(op.Arm);

            int stb = 0;
            long elapsed = 0;
            while (true)
            {
                stb = channel.SerialPoll();            // latched read - bits stay set until polled
                elapsed = nowMs() - start;
            ...
            // Clear the mask and restore prior state (both best effort - never fail the wait over them).
            SafeSend(channel, model.EnableMask.ClearCommand);
            SafeSend(channel, op.Restore);
            ...
            if (err)
                return new CompletionResult(CompletionOutcome.InstrumentError,
            ...
            if (done)
                return new CompletionResult(CompletionOutcome.Completed,
                    "Completed '" + operationName + "' after " + elapsed + " ms - " + detail + ".", stb, elapsed, bits);
            return new CompletionResult(CompletionOutcome.TimedOut,
        ...
        private static void SafeSend(IStatusChannel channel, string command)
        {
            if (string.IsNullOrEmpty(command)) return;
            try { channel.Send(command); } catch { /* best effort */ }
        }
        ...
        private static string PromptOperation(string model, string operation, StatusModel model_)
        {
            string known = (model_.Operations != null && model_.Operations.Count > 0)
                ? "Known operations: " + string.Join(", ", model_.Operations.Keys) + "."
