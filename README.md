...
| `instrument_reference` | `model` | `command`, `search`, `category` | Browse a model's commands, or (with `command=`) get a read/write **recipe**: `read.send` is the exact query string; `write` gives the template + whether to append a suffix token (→ use `resolve_setting`) or send a bare number |
